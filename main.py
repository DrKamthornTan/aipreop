    ...
    if user_en:
        try:
            ...
            if results1:
                ...
                # Display all vital signs recorded
                #st.write("All Vital Signs Data:")
                #st.write(df1)
            ...
            if bmi is not None:
                ptinfo_text = f"{gender} aged (when operated {age} yr), BMI - {bmi}, with diagnosis of - {icd10}, underlying disease - {underlying_disease}, history of allergy - , ASA - {asa}, estimated time of operation - {rbop_estimatedtime} and to undergo: {icd9}"
            else:
                ptinfo_text = f"{gender} aged (when operated {age} yr), BMI - {bmi}, with diagnosis of - {icd10}, underlying disease - {underlying_disease}, history of allergy - , ASA - {asa}, estimated time of operation - {rbop_estimatedtime} and to undergo: {icd9}"
            ...
            prompt = prompt_template.format(context=context_text, question="What are alerts and the risk for the upcoming operation?")
            ...
            # Format the sources with red color
            formatted_sources = "<br>".join([f"<span style='color:red'>{source}</span>" for source in sources if source])
